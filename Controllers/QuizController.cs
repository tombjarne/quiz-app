...
namespace Lifekeys.Controllers
{
    ...
    public class QuizController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetQuestion(string quizId, string questionId)
        {
            ...
            string quizId, string questionId, [FromBody] AnswerSchema answer)
        {
            if (answer.Type == 2)
            {
                return await _quizService.CheckTrueFalseAnswer(questionId, answer.Solution);
            }
            return await _quizService.CheckAnswer(questionId);
        }
